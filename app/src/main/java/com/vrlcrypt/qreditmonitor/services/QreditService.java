...
import com.vrlcrypt.qreditmonitor.models.Peer;
...
import com.vrlcrypt.qreditmonitor.models.Transaction;
import com.vrlcrypt.qreditmonitor.models.Voters;
...
public class QreditService {
    private static QreditService instance;
    ...
    private QreditService() {
        client = new OkHttpClient();
    }

    public static synchronized QreditService getInstance() {
        if (instance == null) {
            instance = new QreditService();
        }
        return instance;
    }
    ...
    public void requestAccount(Settings settings, final RequestListener<Account> listener) {
        ...
        if (!Utils.validateQreditAddress(settings.getQreditAddress())) {
            listener.onFailure(new Exception("Invalid Qredit Address"));
            return;
        }

        String urlRequest = replaceURLWithSettings(ACCOUNT_URL, settings);
        urlRequest = urlRequest + "?address=" + settings.getQreditAddress();
    ...
    public void requestLatestTransactions(Settings settings, final RequestListener<List<Transaction>> listener) {
        ...
        urlRequest = urlRequest + "?senderId=" + settings.getQreditAddress() +
                "&recipientId=" + settings.getQreditAddress() +
